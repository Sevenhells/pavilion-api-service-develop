package com.potevio.xacp.api.common.interceptor;
...
public class LoginInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(StringUtils.isEmpty(authKey) || StringUtils.isEmpty(sessionId)) {
			...
			writer.write(FastJsonUtils.resultError(LocalErrorCode.INTERCEPTOR_AUTHKEY_SESSIONID_NULL, "authKey或sessionId不能为空！", null));
			writer.flush();
			return false;
		} 
		...
		if(sessionAdminUser == null || sessionAdminUser.getStatus().equals(0)) {
			response.setContentType("application/json;charset=UTF-8");
			PrintWriter writer = response.getWriter();
			writer.write(FastJsonUtils.resultError(LocalErrorCode.INTERCEPTOR_ACCOUNT_CANNOT_USE, "账号已被删除或禁用", null));
			writer.flush();
			return false;
		}
