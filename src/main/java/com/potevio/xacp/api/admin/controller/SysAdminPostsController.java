...
import com.potevio.xacp.api.admin.service.SysAdminPostService;
 ...
 */
@RestController
@RequestMapping("/api/v1/auth/admin/posts")
...
public class SysAdminPostsController extends CommonController {
	...
	public String save(@RequestBody(required=false) SysAdminPost record,HttpServletRequest request) {
		...
		if(row == 0) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_POSTS_HANDLE_ERROR, "保存失败", null);
		}
	...
	public String update(@RequestBody(required=false) SysAdminPost record,HttpServletRequest request) {
		int row = sysAdminPostService.save(record);
		if(row == 0) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_POSTS_HANDLE_ERROR, "更新失败", null);
		}
	...
	public String delete(@PathVariable Integer id) {
		...
		if(row == 0) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_POSTS_HANDLE_ERROR, "删除失败", null);
		}
	...
	public String deletes(@RequestBody Map<String, Object> params) {
		...
		if (CollectionUtils.isEmpty(ids)) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_POSTS_HANDLE_IDS_NULL, "操作失败", null);
		}
		try {
