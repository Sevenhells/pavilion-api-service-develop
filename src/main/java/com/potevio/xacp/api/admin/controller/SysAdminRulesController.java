...
import com.potevio.xacp.api.admin.service.SysAdminRuleService;
 ...
 */
@RestController
@RequestMapping("/api/v1/auth/admin/rules")
...
public class SysAdminRulesController extends CommonController {
	...
	public String save(@RequestBody SysAdminRule record,HttpServletRequest request) {
		...
		if(row == 0) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_RULES_HANDLE_ERROR, "保存失败", null);
		}
	...
	public String update(@RequestBody SysAdminRule record,HttpServletRequest request) {
		int row = sysAdminRulesService.save(record);
		if(row == 0) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_RULES_HANDLE_ERROR, "更新失败", null);
		}
	...
	public String delete(@PathVariable Integer id) {
		...
		if(row == 0) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_RULES_HANDLE_ERROR, "删除失败", null);
		}
	...
	public String deletes(@RequestBody Map<String, Object> params) {
		...
		if (CollectionUtils.isEmpty(ids)) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_RULES_HANDLE_ERROR, "操作失败", null);
		}
		try {
			...
			return FastJsonUtils.resultError(LocalErrorCode.COMMON_EXCEPTION, "操作失败", null);
		}
	...
	public String enables(@RequestBody Map<String, Object> params) {
		...
		if (CollectionUtils.isEmpty(ids)) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_RULES_HANDLE_IDS_NULL, "操作失败", null);
		}
		try {
			for (int i = 0; i < ids.size(); i++) {
