...
import com.potevio.xacp.api.admin.service.SysAdminMenuService;
...
public class LoginController extends CommonController {
	...
	public String login(@RequestBody SysAdminUser record,HttpServletRequest request) {
		...
		if(StringUtils.isBlank(record.getUsername())) {
			return FastJsonUtils.resultError(LocalErrorCode.COMMON_USER_NULL, "账号不能为空", null);
		}
		record.setPassword(DigestUtils.md5Hex(record.getPassword()));
		SysAdminUser adminUser = sysAdminUserService.selectOne(record);
		if(adminUser == null) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_ACCOUNT_PASSWORD_ERROR, "帐号与密码错误不正确", null);
		}
		if(!adminUser.getStatus().equals(Byte.valueOf("1"))) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_ACCOUNT_CANNOT_USE, "帐号已被禁用", null);
		}
	...
	public String relogin(String rememberKey,HttpServletRequest request) {
		...
		if(user == null) {
			return FastJsonUtils.resultError(LocalErrorCode.ADMIN_ACCOUNT_NOT_EXIST, "重新登录失败", null);
		}
